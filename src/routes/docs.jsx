...
export const Route = createFileRoute("/docs")({
    head: () => ({ meta: [{ title: "Documentation — Sharexpress Cloud" }] }),
    component: DocsPage,
});
...
const docGuides = {
    "Quickstart": {
        ...
        content: (
            <div className="space-y-3">
                <p className="text-[13px] text-muted-foreground">Get started on the Sharexpress Cloud control plane by installing our CLI, authenticating, and launching your first application cluster.</p>
                <div className="rounded-md border border-border bg-background p-4 font-mono text-[12.5px] text-foreground">
                    <div className="text-muted-foreground"># Install global Sharexpress Cloud Command Line CLI</div>
                    <div><span className="text-accent">$</span> npm i -g @sharexpress/cloud-cli</div>
                    <div className="mt-3 text-muted-foreground"># Login to authenticate with your organization credentials</div>
                    <div><span className="text-accent">$</span> sharexpress login</div>
                    <div className="mt-3 text-muted-foreground"># Trigger a live edge deploy directly from codebase directory</div>
                    <div><span className="text-accent">$</span> sharexpress deploy --prod</div>
                    <div className="mt-3 text-success font-semibold">✓ Deployment active at https://sharexpress.in (edge version v1.0.0)</div>
                </div>
            </div>
        )
    },
    "Concepts": {
        ...
        content: (
            <div className="space-y-3 text-[13px] text-muted-foreground">
                <p>Sharexpress Cloud matches world-class developer experience with a robust service catalog. Here are the core abstractions:</p>
                <ul className="list-disc pl-5 space-y-1">
                    ...
                    <li><strong className="text-foreground">Object Storage:</strong> Media pipelines for assets.sharexpress.in and files.sharexpress.in paired with S3 storage buckets.</li>
                </ul>
            </div>
        )
    },
    "CLI setup": {
        ...
        content: (
            <div className="space-y-3">
                <p className="text-[13px] text-muted-foreground">Setup the CLI securely using environment variables or configuration files for automated CI/CD pipelines.</p>
                <div className="rounded-md border border-border bg-background p-4 font-mono text-[12.5px] text-foreground">
                    <div className="text-muted-foreground"># Authenticate headless using API Key tokens</div>
                    <div><span className="text-accent">$</span> export SHAREXPRESS_TOKEN=sk_live_token_2026</div>
                    <div><span className="text-accent">$</span> sharexpress whoami</div>
                    <div className="mt-2 text-muted-foreground">Logged in as Sharexpress Engineering (Organization: Sharexpress Foundation)</div>
                </div>
            </div>
        )
    },
    "Projects": {
        ...
        content: (
            <div className="space-y-3">
                <p className="text-[13px] text-muted-foreground">Create, list, or terminate deployment projects programmatically.</p>
                <div className="rounded-md border border-border bg-background p-4 font-mono text-[12.5px] text-foreground">
                    <div className="text-muted-foreground">POST https://api.cloud.sharexpress.in/v1/projects</div>
                    ...
                    <div className="text-muted-foreground">{`{ "name": "docs-api", "repo": "sharexpress/docs", "region": "fra1" }`}</div>
                </div>
            </div>
        )
    },
    "TypeScript": {
        ...
        content: (
            <div className="space-y-3">
                ...
                <div className="rounded-md border border-border bg-background p-4 font-mono text-[12.5px] text-foreground">
                     <div><span className="text-accent font-semibold">import</span> {`{ SharexpressCloud }`} <span className="text-accent font-semibold">from</span> <span className="text-success font-semibold">"@sharexpress/sdk"</span>;</div>
                    <div className="mt-1"><span className="text-accent font-semibold">const</span> cloud = <span className="text-accent font-semibold">new</span> SharexpressCloud({`{ token: "sk_live_..." }`});</div>
                    <div className="mt-2 text-muted-foreground">// trigger a serverless function invocation</div>
                    <div><span className="text-accent font-semibold">await</span> cloud.functions.invoke(<span className="text-success font-semibold">"resize-image"</span>, {`{ fileId: "f_12" }`});</div>
                </div>
            </div>
        )
    }
...
const sections = [
    ...
    { icon: Terminal, title: "CLI", desc: "Ship from your terminal with `sharexpress deploy`.", links: ["Install", "Auth"] },
...
function DocsPage() {
    ...
    return (<AppShell breadcrumbs={[{ label: "Sharexpress Cloud" }, { label: "Documentation" }]}>
      <PageShell>
        <PageHeader title="Documentation" description="Everything you need to build and scale apps on Sharexpress Cloud."/>
