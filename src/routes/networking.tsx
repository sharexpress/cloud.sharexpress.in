...
function NetworkingPage() {
  return (
    <AppShell breadcrumbs={[{ label: "Acme Inc" }, { label: "Networking" }]}>
      <PageShell>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <Panel title="Regions" padded={false}>
            <ul className="divide-y divide-border">
              {regions.map((r) => (
                <li key={r.code} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-5 py-3">
          ...
          <Panel title="Private networks">
            <div className="space-y-3">
              {["vpc-prod (10.0.0.0/16)", "vpc-staging (10.10.0.0/16)", "vpc-analytics (10.20.0.0/16)"].map((v) => (
                <div key={v} className="flex items-center justify-between rounded-md border border-border bg-background px-3 py-2 font-mono text-[12px]">
